...
- **🔒 ハイブリッドストレージ**：LocalStorage（高速）+ RESTful API（同期）
...
2. 右下の青いプラスボタンから新規タスクを作成
3. 顧客タブで関連する顧客を登録
4. 各タスクで時間計測を開始
5. ガントタブでプロジェクト全体を可視化
6. レポートタブで進捗と効率を分析
7. **設定タブでGoogle Calendar統合**を設定
...
### データバックアップ
- 設定タブの「データのエクスポート」で全データをJSONファイルとして保存可能
- 「データのインポート」で他のブラウザや端末にデータを移行可能
