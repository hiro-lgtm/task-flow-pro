...
- **タスク件名垂直位置修正**: タスクタイトルが中央より下に表示される問題を解消
  - **line-height調整**: 1.3から1.4に変更して適切な行間を確保
  - **webkit-box-align設定**: `-webkit-box-align: start` で上寄せ配置を明確化
  - **微調整**: `top: -1px` で1ピクセル上方向に微調整
  - **コンテンツ配置統一**: `.task-content` でflexbox上寄せ配置を設定
